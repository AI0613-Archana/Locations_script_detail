...
class hertz_1:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = result["source_url"] or "https://www.hertz.co.za/"
            headers = {
                "accept": (
    ...
    def extraction(
        ...
        pickup = soup.select_one("#pickup-depot")
        return_depot = soup.select_one("#return-depot")
        if not pickup:
            return
        ...
        if return_depot:
            pickup_codes = {
                option.get("value")
                for option in pickup.find_all("option")
                if option.get("value")
            }
            return_codes = {
                option.get("value")
                for option in return_depot.find_all("option")
                if option.get("value")
            }
            if pickup_codes != return_codes:
                print("Pickup and Return locations differ")

        for option in pickup.find_all("option"):
            location_code = re.sub(
                r"\s+", " ", str(option.get("value") or "")
            ).strip()
            if not location_code or location_code in seen_location_codes:
                continue

            pickup_location = re.sub(
                r"\s+", " ", option.get_text(" ", strip=True)
            ).strip()
            if not pickup_location:
                continue

            city = re.sub(
                r"\s+", " ", str(option.get("data-region") or "")
            ).strip()
            depot_type = re.sub(
                r"\s+", " ", str(option.get("data-depot-type") or "")
            ).strip()
            country_code = re.sub(
                r"\s+", " ", str(option.get("data-country") or country or "ZA")
            ).strip()
            optgroup = option.find_parent("optgroup")
            location_country = (
                re.sub(r"\s+", " ", str(optgroup.get("label") or "")).strip()
                if optgroup
                else ""
            )
