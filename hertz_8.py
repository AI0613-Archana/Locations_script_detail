...
class hertz_8:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = result["source_url"] or "https://www.hertz.com.tr/AjaxHandler.ashx"
            headers = {
                "Accept": "application/json, text/javascript, */*; q=0.01",
                "Accept-Language": "en-US,en;q=0.9",
                "Connection": "keep-alive",
                "Referer": "https://www.hertz.com.tr/",
    ...
    def extraction(
        ...
        locations = response_data if isinstance(response_data, list) else []
        ...
        for location in locations:
            ...
            location_code = re.sub(
                r"\s+", " ", str(location.get("Key") or "")
            ).strip()
            location_term = re.sub(
                r"\s+", " ", str(location.get("Value") or "")
            ).strip()
            if (
                not location_code
            ...
            # Try to extract an IATA code in parentheses like "(ABC)"
            code_match = re.search(r"\(([A-Z]{3})\)", location_term)
            if code_match:
                iata_code = code_match.group(1)
            else:
                # Fallback: find any 3-letter uppercase token and validate against known IATA codes
                iata_code = ""
                tokens = re.findall(r"\b[A-Z]{3}\b", location_term)
                for t in tokens:
                    if t in self.iata_codes:
                        iata_code = t
                        break
            is_airport = True if re.search(r"AIRPORT|HAVALIMANI|HAVALİMANI", location_term, re.I) else False
            ...
            city = re.sub(r"\s*\([^)]*\)\s*$", "", location_term).strip()
            city = re.split(r"\s+-\s+|\s{2,}", city, maxsplit=1)[0].strip()
...
if __name__ == "__main__":
    ...
    SC = None
    try:
        # SC = hertz_8(0, 133, 133, "input_locations", "locations", False, "99")
