...
class eldon:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = (
                result["source_url"]
                or "https://main.eldan.co.il/api/branches/skinny/Rent"
            )
            headers = {
                "Accept": "application/json, text/plain, */*",
                "Accept-Language": "en-US,en;q=0.9",
                "Origin": "https://www.eldan.co.il",
                "Referer": "https://www.eldan.co.il/en/rent",
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        print("Direct retry failed:", exc)
                        response = self.load(source_url, headers, {})
                        print("Status:", response.status_code, "second direct retry")
    ...
    def extraction(
        ...
        if response_data.get("errorCode") not in (None, 0):
            return

        locations = response_data.get("body", [])
        if not isinstance(locations, list):
            locations = (
                response_data.get("pageProps", {}).get("mapData", [])
                if isinstance(response_data.get("pageProps"), dict)
                else []
            )
        if not isinstance(locations, list):
            return
        ...
        for location in locations:
            ...
            location_code = re.sub(
                r"\s+", " ", str(
                    location.get("branchNumber")
                    or location.get("branchId")
                    or location.get("id")
                    or ""
                )
            ).strip()
            ...
            address = re.sub(
                r"\s+", " ", str(
                    location.get("enAddress")
                    or location.get("addressEn")
            ...
            branch_name = re.sub(
                r"\s+", " ", str(
                    location.get("name")
                    or location.get("enName")
                    or location.get("branchName")
                    or ""
                )
            ).strip()
            location_name = branch_name or city or address or location_code
            ...
            location_term = " - ".join(
                part for part in (location_name, address) if part and part != location_name
            )
            is_airport = True if re.search(r"airport|gurion|ben gurion|tlv", location_name, re.I) else False
            ...
            row = {
                ...
                "location_country": country or "IL",
...
if __name__ == "__main__":
    SC = None
    try:
        # SC = eldon(0, 159, 159, "input_locations", "locations", False, "20")
