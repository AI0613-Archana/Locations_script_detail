...
SOURCE_NAME = "avecrentacar"
WEBSITE_CODE = 11
DOMAIN = "https://www.avecrentacar.com/"
...
SELECT_PATTERN = re.compile(
    r'<select[^>]*id="pickup_location"[^>]*>(.*?)</select>',
...
class avecrentacar:
    ...
    def extraction(self, html_text, refid, websitecode, source_name):
        ...
        seen_codes = set()
        for location_code, location_name in OPTION_PATTERN.findall(match.group(1)):
            ...
            is_airport = any(
                keyword in lowered_name for keyword in ("havaliman", "airport")
            )
...
if __name__ == "__main__":
    ...
    while RETRY < 20:
        SC = None
        try:
            SC = avecrentacar(0, 153, 153, "input_locations", "locations", False, "20")
            # (
            #     script,
            #     status,
            #     startid,
