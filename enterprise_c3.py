...
COUNTRY_CONFIG = {
    ...
    "CZ": {"countryCode": "CZ", "cor": "CZ", "locale": "cs", "domain": "enterpriserentacar.cz"},
}
...
class enterprise_c3:
    def __init__(
        ...
        self.websitecode = 27           # ← update to Enterprise's actual websitecode for CZ
    ...
    def load(self, ss, bookingcountry, proxies):
        ...
        headers = {
            ...
            'Origin': f'https://www.{cfg["domain"]}',
            'Referer': f'https://www.{cfg["domain"]}/',
        ...
        url = 'https://pbk.partnerbookingkit.com/api/booking/v1/efb631ab135c0/stations'
    ...
    def _load_existing_location_keys(self, cursor):
        ...
        print("Existing locations loaded:", len(keys))
        return keys
        ...
        for attempt, current_proxies in enumerate(attempts, start=1):
            try:
                ...
                if not isinstance(result, dict):
                    raise ValueError("Enterprise response is not a JSON object")

                if result.get("status") != "OK":
                    raise ValueError(f"Enterprise response status is not OK: {result.get('status')}")

                locations_list = result.get("result", [])

                with self.cache_lock:
                    self.api_cache[ck] = locations_list
    ...
    def iter_locations(self, response_data):
        """Yield every location returned by the endpoint."""
        if not isinstance(response_data, list):
            return
        
        for location in response_data:
            if isinstance(location, dict):
                # The response structure for this API is flat inside result
                station_type = location.get("stationType", {}).get("description", "Other")
                yield location, station_type
    ...
    def make_location_code(self, location, default_type):
        """Use code; create a stable fallback only if it is absent."""
        location_code = self.get_first(location, "code")
        ...
        fingerprint = "|".join(
            ...
            for value in (
                ...
                address.get("country", {}).get("code", ""),
            )
        )
        return f"generated-{sha256(fingerprint.encode('utf-8')).hexdigest()[:24]}"

    def build_location_details(self, location, default_type):
        """Map one response item to the shared locations-table fields."""
        address = location.get("address") or {}
        geo = location.get("geoCode") or {}
        
        latitude = self.get_first(geo, "lat")
        longitude = self.get_first(geo, "lng")
        
        location_name = self.get_first(location, "name")
        location_type = self.get_first(location.get("stationType", {}), "description", default=default_type)
        
        city = self.get_first(address, "cityName")
        region = self.get_first(address, "state")
        
        country = address.get("country") or {}
        location_country = self.get_first(country, "code")
        
        is_airport = location.get("atAirport") is True
        airport_code = location.get("iataCode") or self.make_location_code(location, default_type)
    ...
    def extraction(self, item, refid, websitecode, source_name, rows):
        ...
        for location, default_type in self.iter_locations(response_data):
            ...
            # The JSON array returns a large number of airports and cities
            # Only process if it is an airport and matches the queried IATA code.
