...
class hertz_5:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            country = result["country"] or "MA"
            source_url = "https://api.rentalis-prod.digital.infobam.com/api/v1/agencies/agenciesFilter/51"

            headers = {
                "accept": "*/*",
                "accept-language": "en-US,en;q=0.9",
                "content-type": "application/json",
                "origin": "https://www.hertz.ma",
                "priority": "u=1, i",
                "referer": "https://www.hertz.ma/",
                ...
                "x-application-code": "HMA-SW",
                "x-branch-code": "HMA",
                "x-brand-code": "HTZ",
                "x-invisible-token": "AqBCXptBAzX9PSEJ2YSRTxF7ZwhfBJVA3XhF9TvSYnFt9cDwTcgN5Qf9922ShX",
            }
    ...
    def extraction(
        ...
        locations = (
            response_data.get("agencies", []) if isinstance(response_data, dict) else []
        )
        ...
        for location in locations:
            ...
            agency_erp_code = re.sub(
                r"\s+", " ", str(location.get("agencyErpCode") or "")
            ).strip()
            agency_id = re.sub(
                r"\s+", " ", str(location.get("agencyId") or "")
            ).strip()
            location_term = re.sub(
                r"\s+", " ", str(location.get("agencyName") or "")
            ).strip()

            if not agency_erp_code or not location_term:
                continue

            location_code = (
                f"{agency_erp_code}|{agency_id}" if agency_id else agency_erp_code
            )
            if location_code in seen_location_codes:
                continue

            city = re.sub(r"\s+", " ", str(location.get("city") or "")).strip()
            location_country = country or "MA"
            booking_country = country or "MA"
            location_type_source = re.sub(
                r"\s+", " ", str(location.get("agencyTypeCode") or "")
            ).strip()

            is_airport = (
                True
                if location_type_source == "ARP"
                or re.search(r"AIRPORT|AEROPORT|AÉROPORT", location_term, re.I)
                else False
            )
