...
class enterprise_c2:
    ...
    def load(self, ss, bookingcountry, proxies):
        ...
        cookies = {
            '_cfuvid': '2FvCOvCRzcRKIqt9ZygQeL3BwUc5uKtiGVqo1QSnuLg-1788947648.6225402-1.0.1.1-let2cWRLm0kv8WK5r2.61dvZD2E_gmUE7isXBIS46dE',
            '__cf_bm': 'B0DTPzmB9DMa21VOmy2alKGlW6ujqlDysFbpRKHiaQA-1788951421.8642297-1.0.1.1-lf9FL77.ceAAEArQ19n17TDeOd5QCceBduk8_VuAFW3kf_n9HXqIWuYZWkkGH7P.WO6AEOPy72ZR5sD7lGhQcaEr2VG61i3bSeJZlmbn6ugG0fHYWVS2CuB9mjb_IIqp',
            '__cf_bm': 'PNHSoIy14LQNFqUPslpiVQVvcCWM2dnfDu3TPfeEB.Y-1788952070.9139583-1.0.1.1-_5LVwA_WqPxG2M8cwbXLX7EXBMvRfoZyyndOzz_MAnBlx0LnoXy4a6w5OEXPrz1XHv7V3pbeq5seRjW.7rMxy.I_YlXpKdpOFQM7XqNTzCdcTscRgjByZyM4WYS_i7DT',
            '_cfuvid': 'RGrEH6726MkYChiReOFUedHz0YDXBCbTDN03YkLrjDc-1788952070.9139583-1.0.1.1-JeprQDHVnQnf6WdTged4KFPNuCyjcNXv5HC_KV3FL4k',
        }
        ...
        headers = {
            ...
            'priority': 'u=1, i',
            'referer': f'https://www.{cfg["domain"]}/de/',
        ...
        params = {
            'url': f'/v1/search/location/enterprise/web/text/{ss}',
            'query': json.dumps({"locale": cfg["locale"]}),
        }

        url = f'https://www.{cfg["domain"]}/_hcms/api/solr'
    ...
    def _load_existing_location_keys(self, cursor):
        ...
        print("Existing Enterprise locations loaded:", len(keys))
        return keys
        ...
        for attempt, current_proxies in enumerate(attempts, start=1):
            try:
                ...
                if "response" in result:
                    ...
                    raise ValueError("Enterprise response missing 'response' key")
    ...
    def iter_locations(self, response_data):
        """Yield every location category returned by Enterprise's typeahead endpoint."""
        location_groups = {
            "airports": "Airport",
            "cities": "City",
            "branches": "Branch",
    ...
    def make_location_code(self, location, default_type):
        """Use Enterprise's station_id; create a stable fallback only if it is absent."""
        location_code = self.get_first(
            location, "station_id", "stationId", "location_id", "locationId"
        )
    ...
    def build_location_details(self, location, default_type):
        """Map one Enterprise response item to the shared locations-table fields."""
        address = location.get("address") or {}
