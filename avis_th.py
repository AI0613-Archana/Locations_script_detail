...
class avis_7:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = result["source_url"] or "https://www.avisthailand.com/"
            headers = {
    ...
    def extraction(
        ...
        for attrs, option_text in option_matches:
            value_match = re.search(r'value=["\']([^"\']*)["\']', attrs, flags=re.I)
            location_code = re.sub(
                r"\s+", " ", unescape(value_match.group(1) if value_match else "")
            ).strip()
            location_term = re.sub(
                r"\s+",
                " ",
                unescape(re.sub(r"<[^>]+>", "", option_text)),
            ).strip()
            if (
                not location_code
                ...
                continue
            if "disabled" in attrs.lower() or "PICK-UP LOCATION" in location_term.upper():
                continue
            ...
            iata_code = ""
            if is_airport:
                normalized_term = re.sub(r"[^A-Z0-9]+", "", location_term.upper())
                known_iatas = {
                    "SUVARNABHUMI": "BKK",
                    "DONMUANG": "DMK",
                    "DONMUEANG": "DMK",
                ...
                for airport_key, airport_iata in known_iatas.items():
                    if airport_key in normalized_term:
                        iata_code = airport_iata
                        break
                if not iata_code:
                    for airport_iata, airport_data in self.airports.items():
                        if airport_data.get("country") != "TH":
                            continue
                        airport_name = re.sub(
                            r"[^A-Z0-9]+",
                            "",
                            str(airport_data.get("name") or "").upper(),
                        )
                        airport_city = re.sub(
                            r"[^A-Z0-9]+",
                            "",
                            str(airport_data.get("city") or "").upper(),
                        )
                        if (
                            airport_name
                            and airport_name.replace("INTERNATIONAL", "") in normalized_term
                        ) or (airport_city and airport_city in normalized_term):
                            iata_code = airport_iata
                            break

            location_name = iata_code if is_airport and iata_code else location_term
            pickup_location = location_name
            city = re.sub(r"\s+-\s+AVIS\s+PARTNER\s*$", "", location_term, flags=re.I).strip()
...
if __name__ == "__main__":
    ...
    SC = None
    try:
        # SC = avis_7(0,175, 175, "input_locations", "locations", False, "20")
