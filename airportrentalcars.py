...
class airportrentalcars:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = (
                result["source_url"] or "https://www.airportrentalcars.com/landing/"
            )
            headers = {
    ...
    def extraction(
        ...
        preloaded_data = json.loads(html[start:end])
        airports = preloaded_data.get("airports", {})
        if not isinstance(airports, dict):
            return

        for airport_group, default_country in (
            ("usAirports", "US"),
            ("internationalAirports", ""),
        ):
            for location in airports.get(airport_group, []):
                ...
                city = city_value
                region = ""
                location_country = default_country
                if "," in city_value:
                    city, region = [part.strip() for part in city_value.split(",", 1)]
                    if not location_country:
                        location_country = region
                if not location_country:
...
if __name__ == "__main__":
    SC = None
    try:
        # SC = airportrentalcars(0, 157, 157, "input_locations", "locations", False, "20")
