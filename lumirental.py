...
class lumirental:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = result["source_url"] or "https://lumirental.com/ar/"
            headers = {
                "accept": (
    ...
    def extraction(
        ...
        response_data = json.loads(script.string)
        queries = (
        ...
        for query in queries:
            ...
            if not query_key or query_key[0] != "branches":
                continue
            ...
            for branch in branches:
                ...
                location_code = re.sub(
                    r"\s+", " ", str(branch.get("id") or "")
                ).strip()
                name_data = branch.get("name") if isinstance(branch.get("name"), dict) else {}
                location_name = re.sub(
                    r"\s+", " ", str(name_data.get("en") or name_data.get("ar") or "")
                ).strip()
                if (
                    not location_code
                ...
                branch_type = re.sub(
                    r"\s+", " ", str(branch.get("type") or "")
                ).strip()
                city_data = branch.get("city") if isinstance(branch.get("city"), dict) else {}
                city_name_data = (
                    city_data.get("name") if isinstance(city_data.get("name"), dict) else {}
                )
                city = re.sub(
                    r"\s+", " ", str(city_name_data.get("en") or city_name_data.get("ar") or "")
                ).strip()
                is_airport = True if branch_type.upper() == "AIRPORT" else False
...
if __name__ == "__main__":
    ...
    SC = None
    try:
        # SC = lumirental(0, 164, 164, "input_locations", "locations", False, "20")
