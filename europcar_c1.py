...
class europcar:
    ...
    def get_proxy(self):
        ...
        proxy_str = 'rentalcarspxys:SyaP2wdkIHybBt6W_country-uk@geo.iproyal.com:12321'
        print(proxy_str)
        ...
        if html:
            ...
            for data1 in self.jsonMatch("allCoutriesStations", html):
                for data in self.jsonMatch(data1, html["allCoutriesStations"]):
                    ...
                    iata_code = locationid[:3]
                    locationterm = self.jsonMatch("label", data)
                    locationname = self.jsonMatch("stationName", data["details"])
                    address1 = self.jsonMatch("street", data["details"])
                    city = self.jsonMatch("city", data["details"])
                    location_country = self.jsonMatch("country", data)
                    ...
                    downtown = self.jsonMatch("downtown", data)
                    railway = self.jsonMatch("railway", data)
                    port = self.jsonMatch("port", data)
                    ...
                    row = {
                        ...
                        "latitude" : latitude,
                        "longitude" : longitude,
                        "booking_country" : location_country,
                    }
                    rows.append(row)
            self.insert(rows)
            self.update(1, refid)
...
if __name__ == "__main__":
    ...
    while RETRY < 20:
        SC = None
        try:
            SC = europcar(0, 2, 2, "input_locations", "locations", False, "60")
            # (
            #     script,
            #     status,
            #     startid,
