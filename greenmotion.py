...
class greenmotion:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            source_url = result["source_url"] or "https://greenmotion.com/locations"
            headers = {
                "accept": (
    ...
    def extraction(
        ...
        match = re.search(r'<locations\b[^>]*\blocations="([^"]*)"', html, re.S)
        if not match:
            return
        ...
        for location in response_data:
            ...
            location_country = re.sub(
                r"\s+", " ", str(location.get("title") or country or "")
            ).strip()
            ...
            for entry in entries:
                ...
                location_name = re.sub(
                    r"\s+", " ", str(entry.get("title") or "")
                ).strip()
                if "airport" not in location_name.lower():
                    continue

                location_code = re.sub(
                    r"\s+", " ", str(entry.get("locationId") or "")
                ).strip()
                if (
                    not location_code
...
if __name__ == "__main__":
    ...
    SC = None
    try:
        # SC = greenmotion(0, 168, 168, "input_locations", "locations", False, "20")
