...
class maggiore:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            headers = {
                ...
                "Referer": "https://www.maggiore.it/en/",
    ...
    def extraction(
        ...
        locations = response_data.get("agenzie", {})
        ...
        for location_key, location in locations.items():
            ...
            location_code = re.sub(
                r"\s+", " ", str(location.get("id") or location_key or "")
            ).strip()
            location_label = re.sub(
                r"\s+",
                " ",
                str(location.get("Label") or location.get("desc") or ""),
            ).strip()
            if (
                not location_code
            ...
            if is_airport:
                ...
                pickup_location = location_label
                location_type = "Railway Station"
            else:
                pickup_location = location_label
                location_type = "City"

            city = re.sub(r"\s+", " ", str(location.get("loc") or "")).strip()
            region = re.sub(r"\s+", " ", str(location.get("area") or "")).strip()
            ...
            row = {
                ...
                "location_country": country or "IT",
