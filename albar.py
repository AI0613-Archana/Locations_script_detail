...
class albar:
    ...
    def main(self, resultset):
        for result in resultset:
            ...
            country = result["country"] or "IL"
            source_url = (
                result["source_url"]
                or "https://rent.albar.co.il/umbraco/surface/SearchEngine/GetBranches"
            )
            headers = {
                "accept": "application/json, text/plain, */*",
                "accept-language": "en-US,en;q=0.9",
                "displaylanguage": "en",
                "referer": "https://rent.albar.co.il/en/rent-in-israel/",
    ...
    def extraction(
        ...
        for location in locations:
            ...
            location_code = re.sub(
                r"\s+", " ", str(location.get("Id") or location.get("id") or "")
            ).strip()
            location_name = re.sub(
                r"\s+", " ", str(location.get("Name") or location.get("name") or "")
            ).strip()
            if (
                not location_code
            ...
            city = re.sub(
                r"\s+", " ", str(location.get("City") or location.get("CityCode") or "")
            ).strip()
            region = re.sub(r"\s+", " ", str(location.get("LotArea") or "")).strip()
...
if __name__ == "__main__":
    SC = None
    try:
        # SC = albar(0, 155, 155, "input_locations", "locations", False, "20")
