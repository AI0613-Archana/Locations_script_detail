...
BASE_URL = "https://pbk.partnerbookingkit.com/api/booking/v1/a99aef6c12422/stations"
...
COUNTRY_CONFIG = {
    ...
    "IT": {"countryCode": "IT", "cor": "IT", "locale": "en-US", "domain": "www.alamo.it"},
    # "JP": {"countryCode": "JP", "cor": "JP", "locale": "en-US", "domain": "www.alamo.jp"},
    # "FI": {"countryCode": "FI", "cor": "FI", "locale": "en-US", "domain": "www.alamo.fi"},
    # "SE": {"countryCode": "SE", "cor": "SE", "locale": "en-US", "domain": "www.alamo.se"},
    # "CH": {"countryCode": "CH", "cor": "CH", "locale": "en-US", "domain": "www.alamo.ch"},    
}
...
class alamo:
    ...
    def load(self, ss, bookingcountry, proxies):
        cfg = COUNTRY_CONFIG[bookingcountry]
        params = {
            'uid': '6a9a90e1170d9824',
        }
        
        json_data = {
            'locale': cfg["countryCode"].lower(),
            'query': ss,
        }
    ...
    def iter_locations(self, response_data):
        ...
        locations = response_data.get("result") or []
        if not isinstance(locations, list):
            return
        for location in locations:
            if isinstance(location, dict):
                default_type = "City"
                if location.get("atAirport"):
                    default_type = "Airport"
    ...
    def make_location_code(self, location, default_type):
        ...
        location_code = self.get_first(
            location, "code", "id", "station_id", "stationId", "location_id", "locationId"
        )
    ...
    def build_location_details(self, location, default_type):
        ...
        address = location.get("address") or {}
        country = address.get("country") or {}
        geo_code = location.get("geoCode") or {}
        
        latitude = self.get_first(geo_code, "lat")
        longitude = self.get_first(geo_code, "lng", "long")
        location_name = self.get_first(location, "name")
        
        location_type = "Airport" if location.get("atAirport") else "none"
        
        city = self.get_first(address, "cityName")
        region = self.get_first(address, "state")
        location_country = self.get_first(country, "code")
        
        is_airport = bool(location.get("atAirport")) or default_type == "Airport"
